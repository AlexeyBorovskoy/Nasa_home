...
- **Для внешнего доступа использовать реализованный VPS reverse SSH tunnel** (ADR-0005, `docs/decisions/ADR-0005-vps-autossh-reverse-tunnel.md`) — пробивает CGNAT, не трогает Amnezia. Tailscale (ADR-0004) оставлен как архивный/альтернативный план, но не является текущей реализацией.
