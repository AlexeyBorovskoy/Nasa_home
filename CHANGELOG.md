...
- Monitoring stack analysis and documentation (`docs/17_MONITORING_OBSERVABILITY.md`) — planned
- Docker Compose for monitoring stack (Netdata + Uptime Kuma + Portainer)
- ADR-0002 (storage design), ADR-0003 (networking), ADR-0004 (Tailscale)
- Tailscale deployment plan (`docs/plans/TAILSCALE_ACCESS_PLAN.md`)
- Full operational bash scripts: `backup_databases.sh`, `restic_backup_example.sh`, `docker_health.sh`, `storage_health.sh`, `docker_update_plan.sh`, `network_health.sh`
- GitHub infrastructure: `.github/` issue templates, CI workflows, CODEOWNERS
- `CODE_OF_CONDUCT.md` (Contributor Covenant v2.1, bilingual RU/EN)

### Changed / Изменено

- `README.md` переписан по стандартам GitHub open-source проектов: badges, двуязычность, ASCII-диаграмма, таблицы стека и документации, Quick Start / README.md rewritten to GitHub open-source standards: badges, bilingual sections, ASCII architecture diagram, stack and docs tables, Quick Start
- `docs/16_GITHUB_PUBLICATION.md` дополнен: GitHub Actions, Issue templates, checklist перед публичным релизом / updated with GitHub Actions, Issue templates, pre-release checklist
