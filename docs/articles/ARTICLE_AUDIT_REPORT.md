...
**Оценка готовности к публикации: 8/10.** Скриншоты web UI и Android готовы. Недостаёт: Portainer/Beszel/Uptime Kuma/Swagger (LAN-only, снять дома), off-site backup, USB drama и Talk/API секции добавлены в habr_draft.md.
...
- [ ] Скриншот Portainer — 13 контейнеров up/healthy *(LAN only, нужно дома)*
- [ ] Скриншот Beszel Hub — CPU/RAM/Disk графики Jetson *(http://193.8.215.130:8091)*
- [ ] Скриншот Uptime Kuma — 5 мониторов (все green) *(LAN only :3001)*
- [ ] Скриншот NASA API Swagger UI *(LAN :8099/docs или VPS :8099/docs)*
